...
def get_tics(pth):
    ...
    with open(pth, mode='rt') as fobj:
        dict = {}
        for line in fobj:
            exchange, tic = line.strip().replace('\"', '').lower().split('=')
            dict[tic] = exchange
        return dict
        ...
        if tickers_lst == []:
            ...
            for i in tickers_lst:
                if i not in list(tic_exchange_dic.keys()):
                    raise Exception(f"{i} is not a key of tic_exchange_dic.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    dict = {}
    tic_exchange_data_dict = {}
    data = []
    for tic in tickers_lst:
        tic_exchange_data_dict['exchange'] = tic_exchange_dic[tic]
        lines = read_dat(tic)
        for line in lines:
            data_dic = {}
            for col in col_lst:
                data_dic[col] = line_to_dict(line)[col]
            data.append(data_dic)
        tic_exchange_data_dict['data'] = data
        dict[tic] = tic_exchange_data_dict
    return dict
...
# ----------------------------------------------------------------------------
    ...
    Explain why to configure as in step 1.
        1. This module defines three constants: ROOTDIR, DATDIR and TICPATH.
           These will hold the path to the toolkit project folder, the project1 package, 
           the data sub-folder and the file with the tickers.
        2. Definition of these constants makes it easy for us to access later in this module.
           Running through configuration files can give us control and flexibility.
           We can put settings in configuration files, eliminating the need to recompile 
           an application every time a setting changes.

    Explain which one of the two hypotheses is more likely to be true.
    Please evaluate both hypotheses.    
        The hypotheses 'the articles journalists wrote are based on valuable 
        information beyond firm fundamentals' is more likely to be true.
        1. If the articles journalists wrote are based on investors' evaluations 
        of those firms and contain relatively more negative words, the articles 
        would negatively affect investors’ willingness to invest in a company and 
        their forecasts of future performance, therefore the investors' confidence 
        would reduce and stock returns would decrease in the short run. However, 
        in the long run, the investors' evaluations would change and stock returns 
        may reverse.
        2. If the articles journalists wrote are based on valuable information 
        beyond firm fundamentals, such as political, legal and other non-financial 
        information, and contain relatively more negative words, these non-financial 
        information disclosures are bad signals that the firms cannot meet the 
        expectations and needs of their investors and even the whole society. 
        Non-financial information makes investors better understand the firms' business 
        operations and the results of policy. Besides, valuable information beyond 
        firm fundamentals can be used for long-term value creation and implications 
        for future trends. Therefore, stock returns decrease in the short run with 
        no reversal in the long run.
        
    Given the hypothesis you have chosen,
    evaluate the short-run predictability for the trading volume with reason(s).
        Since valuable information beyond firm fundamentals can be used for 
        long-term value creation and implications for future trends, investors and
        analysts expect stock returns to decrease due to journalists' bad views 
        about the firm performance. People tend to short the position, therefore 
        trading volume would increase in the short run. Markets with larger trading 
        volume are more predictable.
    """
